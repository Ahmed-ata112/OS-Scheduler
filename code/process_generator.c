...
int main(int argc, char * argv[])
{
    ...
    Initializer.algo = (short)atoi(argv[2]);
    Initializer.arg = 0;
    if (argc == 4)
    {
    ...
    //create the queue id
    key_t qid = ftok("keyfile",65);
    msgq_id = msgget(qid,0666|IPC_CREAT);
    if (msgq_id == -1)
    {
        printf("error has been occured in the message queue\n");
        exit(-1);
    }
    ...
    if (send_val == -1)
    {
        printf("error has been occured in scheduler initation operation\n");
        exit(-1);
    }
    ...
    while (ProcessIterator < ProcessesNum)
    {
        if (getClk()>=Processes[ProcessIterator].arrival)
        {
            ...
            if (send_val == -1)
            {
                printf("error has been occured while sending a process number %d to the schedular\n",ProcessIterator);
            }
            ProcessIterator++;
        }
    }   
